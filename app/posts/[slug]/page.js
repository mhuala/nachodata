...
const getOtherPosts = async (postsList, actualPost) => {
    let firstPost = await getRandomElement(postsList.filter((post) => post.id !== actualPost.id))
    let secondPost = await getRandomElement(postsList.filter((post) => post.id !== firstPost.id))
    ...
    return (
        <div className="mx-auto w-11/12 md:w-10/12 lg:w-9/12 mt-12 bg-slate-700 rounded-t-2xl space-y-6">
            ...
            <span className="flex flex-row gap-4 px-10 items-center">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" fill="gray" viewBox="0 0 24 24" strokeWidth={1.5} stroke="gray" className="w-4 h-4">
            ...
            {/* RECOMMENDED POSTS */}
            <RecommendedPost title={recommendedPosts.firstPost.title}
            ...
            />
            <RecommendedPost title={recommendedPosts.secondPost.title}
