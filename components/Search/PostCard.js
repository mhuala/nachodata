...
const PostCard = ({title,date,slug,tags,description,author,bannerURL}) => {
    return (
        <div className="flex flex-col text-white border-gray-300 border rounded-xl text-center px-4 py-4 justify-between h-full bg-pagegray">
        <Link href={`/posts/${slug}`} className="space-y-4" passHref>
         {/* <!-- IMAGE -->  */}
        <div className="relative overflow-hidden cursor-pointer rounded-xl ">
            <Image src={bannerURL} alt={title} width="0" height="0" className="rounded-xl object-cover hover:scale-110 opacity-50 hover:opacity-75 transition duration-300 ease-in-out w-full"/>
                <div className="absolute top-5 left-5">
                  <div className="flex justify-end gap-2 bg-white rounded-full px-2 py-2 shadow-2xl">
                    {tags.map((tag)=>{return(
                      <div key={tag} className="">
                        <Image src={IMAGES_DICTIONARY[tag]} alt={tag} width="0" height="0" className="h-4 w-4 sm:h-6 sm:w-6 md:h-8 md:w-8 xl:h-10 xl:w-10 rounded-full"/>
                      </div>
                    )})}
                  </div>
                </div>
       ...
       {/* <!-- TITLE -->  */}
        <h2 className="text-left sm:text-xl lg:text-2xl text-indigo-400 font-rale">{title}</h2>
        {/* <!-- CONTENT -->  */}
        <p className="line-clamp-3 text-sm sm:text-base lg:text-lg text-gray-300 text-left overflow-ellipsis h-16">{description}</p>
        ...
        <span className="font-mono w-full flex gap-2">By <h6 className="underline">{author}</h6>· {date} ·<TimeIcon/> 2 min read</span>
        </section>
        </Link>
      </div>
